...
async def create_upload_file(request: Request, upload_file: UploadFile):
    try:
        # Абсолютный путь или относительный - не важно, главное создать
        os.makedirs("taximetr/files", exist_ok=True)
    ...
    return PlainTextResponse(f"{base_url}/user_agreement/{upload_file.filename}")
